...
$$
\hat{Y} = b_0 + b_1 Qtr1 + b_2 \cdot Qtr2 + b_3 \cdot Qtr3 + b_4 \cdot \cdot t 
$$
...
- Suppose we learn that electric power consumption in September is down 3% from the previous month. We must be careful with such information, because whenever a seasonal influence is present, such comparisons may be misleading if the data have not been deseasonalized. 
...
The graph of the deseasonalized smartphone sales time series shown in **Figure 17.25** appears to have an upward linear trend.
