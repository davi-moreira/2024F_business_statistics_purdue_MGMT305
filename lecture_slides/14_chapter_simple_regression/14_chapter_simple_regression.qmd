...
# Simple Linear Regression Model {background-color="#cfb991"}

## Simple Linear Regression 

XXX Falar de correlação... comparar com modelagem: ver arquivo 07-01-ds4ps-hyp-test-regression-presentation.html

XXX Motivation: walmark example book - p. 606
...
## Random Error ($\varepsilon$) and Observed Residual ($e$)
...
::: {.box .shadow}
$$
\varepsilon_i \text{ follows Normal } (0, \sigma^2), \text{ for all } i, \text{ independently}.
$$
:::
:::
...
Validate model assumptions by a residual analysis (later).
...
$$
s^2 = \text{MSE} = \frac{\text{SSE}}{(n-2)} \quad \text{Estimated unexplained variation.}
$$

where:

$$
\text{SSE} = \sum e_i^2 = \sum (y_i - \hat{y}_i)^2 = \underline{\hspace{4cm}}
$$
...
$$
s = \sqrt{MSE} = \sqrt{\frac{SSE}{n - 2}} = \underline{\hspace{4cm}}
$$
...
- $\varepsilon_i$ follows Normal (0, $\sigma^2$), for all $i$, independently.

- **(Constant variance)** Random errors have a constant variance.
  - Plot residuals against $x$ or the predicted $y$.

- **(Randomness)** Random errors are random (no special patterns or trends).
  - Plot residuals against observation number.

- **(Normality)** Random errors follow a Normal distribution.
...
If the assumption that the variance of $\varepsilon$ is the same (constant) for all values of $x$ is valid, and the assumed regression model is an adequate representation of the relationship between the variables, then the residual plot should give an overall impression of a horizontal band of points.
:::
...
```{r  echo=FALSE, out.width = "70%",fig.align="center"}
knitr::include_graphics("figs/armands_pizza_plot_residual.png") 
```
...
- **Detecting Outliers**
    ...
    - **Note:** The standardized residual rule sometimes fails to identify an unusually large observation as being an outlier.

:::

## Remedies for Special Patterns
...
```{r  echo=FALSE, out.width = "99%",fig.align="center"}
knitr::include_graphics("figs/residuals_special_pattern_01.png") 
```
...
```{r  echo=FALSE, out.width = "99%",fig.align="center"}
knitr::include_graphics("figs/residuals_special_pattern_02.png") 
```
...
```{r  echo=FALSE, out.width = "99%",fig.align="center"}
knitr::include_graphics("figs/residuals_special_pattern_03.png") 
```

Investigate, or add $x^2$
...
```{r  echo=FALSE, out.width = "99%",fig.align="center"}
knitr::include_graphics("figs/residuals_special_pattern_04.png") 
```

Heteroscedasticity; Transform y to $log(y)$
...
::: columns
::: {.column width="45%" style="text-align: center; justify-content: center; align-items: center;"}

- **$H_0$**: Random Error is normal;
- **$H_1$**: Not normal

:::
::: {.column width="65%" style="text-align: center; justify-content: center; align-items: center;"}
...
## Example: Measuring Stock Risk

<br>

In finance, one measure of the **risk** or **volatility** of an individual stock is the standard deviation of its total return (capital appreciation plus dividends) over several periods of time.

- But the standard deviation does not **take into account** the extent to which the price of a given stock varies as a function of a standard market index, such as the S&P 500.

- **Beta for a stock** is the regression coefficient ($\beta_1$) in a regression model, where the independent variable ($x$) is the total return for the stock market and the dependent variable is the stock’s returns.

    - **Beta greater than 1** indicates that the stock is more volatile than the market and hence a risky stock.
    
    - **Beta less than 1** indicates that the stock is less volatile than the market and hence a less risky stock.
...
```{r  echo=FALSE, out.width = "99%",fig.align="center"}
knitr::include_graphics("figs/beta_stocks_data.png") 
```

## Questions


- Which stocks are the most volatile? (Standard deviation)

- Which stocks would you expect to perform best in an up market? Down market? ($\beta_1$)

- How much of the return for an individual stock is explained by the market? ($R^2$)
...
| Exxon Mobil        | 36    | 0.01664 | 0.05534 | -0.11646 | 0.01278  | 0.23217  |
| Caterpillar        | 36    | 0.0301  | 0.0686  | -0.1006  | 0.0408   | 0.2185   |
| Johnson & Johnson  | 36    | 0.00530 | 0.03487 | -0.05917 | -0.00148 | 0.10334  |
| McDonalds          | 36    | 0.0245  | 0.0681  | -0.1144  | 0.0370   | 0.1826   |
| **SanDisk**            | 36    | 0.0693  | **0.1954**  | -0.2833  | 0.0741   | 0.5017   |
| Qualcomm           | 36    | 0.0284  | 0.0862  | -0.1217  | 0.0387   | 0.2105   |
| Procter & Gamble   | 36    | 0.01059 | 0.03707 | -0.05365 | 0.01333  | 0.08783  |
| S&P 500            | 36    | 0.01010 | 0.02633 | -0.03429 | 0.01034  | 0.08104  |

Using the standard deviation as a measure of volatility, SanDisk was the most volatile stock with a standard deviation of 0.1954.  The stocks of Johnson & Johnson and P & G exhibit less volatility than the other individual stocks. But, all of the individual stocks are more volatile than the market as a whole. The diversification embodied in the S&P 500 reduces its volatility.


## Betas ($\beta_1$) of Stocks
...
| SanDisk            | 2.600 |
| Qualcomm           | 1.410 |
| Procter & Gamble   | 0.507 |


- Any stock with a beta greater than 1 will move up faster than the market when the market goes up. Any stock with a beta less than 1 will not go down as fast as the market in periods where the market declines.

- We would expect SanDisk (+2.600) to benefit most from an up market.  Johnson & Johnson (0.009) is least affected by the market.  The effect of the market going down cannot be expected to exert much downward pressure on shares of Johnson & Johnson.




## Variation Explained by Market

| Company            | Equation                             | R-Sq     |
|--------------------|--------------------------------------|----------|
| Microsoft          | 0.00040 + 0.458 S&P 500              | 7.1%     |
| Exxon Mobil        | 0.00926 + 0.731 S&P 500              | 12.1%    |
| Caterpillar        | 0.015000 + 1.49 S&P 500              | 32.9%    |
| Johnson & Johnson  | 0.00521 + 0.009 S&P 500              | 0.0%     |
| McDonald’s         | 0.00930 + 1.500 S&P 500              | **33.8%**|
| Sandisk            | 0.04300 + 2.600 S&P 500              | 12.3%    |
| Qualcomm           | 0.01410 + 1.410 S&P 500              | 18.7%    |
| Procter & Gamble   | 0.00548 + 0.507 S&P 500              | 12.9%    |


- $R^2$ values for each stock indicate the percentage of variation explained by the market.
